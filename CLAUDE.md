...
- **2026-08-10:** Paua grid-fix (tag `checkpoint-voor-paua-grid-fix` als rollback-punt). **Hoofdfoto gewisseld** naar `paua-2.jpg` (het wijde hangende shot, 1200×1600) — `paua.jpg` (947×1600, zonlicht-shot) sneed te ver in binnen de kleine cel. Galerijvolgorde nu `paua-2, paua, paua-3, paua-4, paua-5`; `images[0] === imageSrc` blijft daarmee kloppen. Detail-hero én OG/twitter-image volgen automatisch mee (`paua-2.jpg`). **Nieuw veld `gridSize?: "tall"`** op PortfolioItem: geeft een item een hoge cel (`md:row-span-2`, `isLarge: true`) **buiten** de bloemen-bento om. Nodig naast het bestaande `gridCell?: "wide" | "tall"` omdat `gridCell` alléén binnen de `bloemen && featured`-tak wordt gelezen — Paua is `featured: false` + `sculpturen` en viel dus door naar de `col-span-1 row-span-1`-fallback. De `gridSize`-check staat ná de ongewijzigde `bloemen && featured`-tak en vóór de categorie-fallback, dus alle bloemen-cellen blijven exact hetzelfde (musa-basjoo 2×2, anthurium/zonnebloem wide, klaproos/ridderspoor/tulpenboeket tall). `cardPosition: "50% 35%"` ongemoeid gelaten: bij aspect 0.75 in de tall-cel (~0.565) is de crop horizontaal en valt de schelp (x≈240-815 van 1200) volledig binnen beeld; op mobiel (0.83) is de verticale overflow ~10% en blijft de schelp ook heel. Spotlight blijft Musa Basjoo.
